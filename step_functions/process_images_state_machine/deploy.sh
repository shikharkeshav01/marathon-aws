...
# Usage: deploy.sh [region] [sfn_role_arn] [list_images_handler_arn] [extract_bib_number_handler_arn] [index_faces_handler_arn] [image_processing_completion_handler_arn]
...
if [[ $# -eq 0 ]]; then
  ...
  INDEX_FACES_HANDLER_ARN="$(aws lambda get-function --function-name index_faces_handler --region "${REGION}" --query 'Configuration.FunctionArn' --output text 2>/dev/null || echo "")"
  ...
  if [[ -z "${LIST_IMAGES_HANDLER_ARN}" ]] || [[ -z "${EXTRACT_BIB_NUMBER_HANDLER_ARN}" ]] || [[ -z "${INDEX_FACES_HANDLER_ARN}" ]] || [[ -z "${IMAGE_PROCESSING_COMPLETION_HANDLER_ARN}" ]]; then
  ...
  INDEX_FACES_HANDLER_ARN="${5}"
  IMAGE_PROCESSING_COMPLETION_HANDLER_ARN="${6}"
fi
...
INDEX_FACES_HANDLER_ARN=$(printf '%s' "${INDEX_FACES_HANDLER_ARN}" | tr -d '\n\r\t ')
...
definition=$(echo "${definition}" | sed "s|\${INDEX_FACES_HANDLER_ARN}|${INDEX_FACES_HANDLER_ARN}|g")
